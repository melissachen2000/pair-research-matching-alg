...
argparser.add_argument("--ratings_folder", required=True)
...
for filename in os.listdir(args.ratings_folder):
    if ".csv" not in filename:
        print(f"Skipping {filename} because it's not a csv file")
        continue

    ratings = pd.read_csv(f'{args.ratings_folder}/{filename}')
